...
// meanSpeed возвращает среднюю скорость тренировки.
...
type InfoMessage struct {
	...
	Distance     float64       // дистанция, которую преодолел пользователь
...
func (r Running) Calories() float64 {
	...
	return (CaloriesMeanSpeedMultiplier*meanSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Seconds() / 3600
}
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(meanSpeed, 2)/w.Height*CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Seconds() / 3600
}
...
func (s Swimming) Calories() float64 {
	...
	return (meanSpeed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Seconds() / 3600
}

// TrainingInfo возвращает структуру InfoMessage с информацией о проведенной тренировке.
func (s Swimming) TrainingInfo() InfoMessage {
	// Пересчитываем информацию о тренировке с использованием переопределенной функции meanSpeed
	info := s.Training.TrainingInfo()
	info.Speed = s.meanSpeed() // Используем переопределённую функцию
	return info
}
